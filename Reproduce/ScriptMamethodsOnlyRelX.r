###This code is an adaptation of the code provided by Predone, 2024 
#to preform the methods described in Ma et al. 2019 in case of two treatment
# this script is used to produce the supplementary results of Ma methods 
...
Mafunction=function(Datalistrep, Dpred=DP, method="hc",RELEVANTVARLIST=relvarindcescommon, CORES=10){
  source("~/Sim/RFunctionsMultiTreatMVBPR.R") #"~/Documents/UNIFI/PhD/Peoject/Code/RFunctionsMultiTreatMVBPR.R")
  
  namerep=Datalistrep$repname
  
  DATA=Datalistrep$Data[,]

  ld=MultiTreatmentMVBPR(DATA, DatasetPostPred=DP, nameTreat="asstrt", 
                         namesPred = RELEVANTVARLIST, nameY="outcome")
...
for (S in Scen) {
  ...
  ##relevant variables index 
  relvarindcescommon=which((LISTDATA$trueparam$ClusterStrInfo$Groupvar) %in% LISTDATA$trueparam$ClusterStrInfo$RelGroupVars)
  
  relvarindcescommon=paste0("d",relvarindcescommon)
  ...
  for (rep in 1:50) {
    ...
    for(MEH in c("hc","pam")){#hc pam
      ...
      mptosaveread="~/Sim/Scen"#   "~/Documents/UNIFI/PhD/Peoject/Results/Scen/"
      scen=S
      namerep=LISTDATA[[paste0("rep",rep)]]$repname
      method=MEH
      ##save results
      cat(paste0(mptosaveread,"/results",scen,"/",namerep,"_ma-",method ))
      saveRDS(Estim,paste0(mptosaveread,"/results", scen,"/",namerep,"_maONLYrel-",method ))
    }
